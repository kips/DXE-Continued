do
	...
	local data = {
		...
		userdata = {
			...
			portaltext = {format(L.alerts["%s Soon"],L.alerts["Portals"]),format(L.alerts["Next %s"],L.alerts["Portals"]), loop = false},
		...
		alerts = {
			...
			-- Phase 1
			fervorwarn = {
				varname = format(L.alerts["%s on self"],SN[63138]),
				type = "simple",
				text = format("%s: %s!",SN[63138],L.alerts["YOU"]),
				time = 2,
				sound = "ALERT4",
				color1 = "PURPLE",
				flashscreen = true,
			...
			blessingwarn = {
				varname = format(L.alerts["%s on self"],SN[63134]),
				type = "simple",
				text = format("%s: %s!",SN[63134],L.alerts["YOU"]),
				time = 2,
				sound = "ALERT8",
				color1 = "CYAN",
				flashscreen = true,
			...
			-- Phase 2
			brainlinkdur = {
				varname = format(L.alerts["%s on self"],SN[63802]),
				type = "centerpopup",
				text = format("%s: %s!",SN[63802],L.alerts["YOU"]),
				time = 30,
				flashtime = 30,
				color1 = "BLUE",
				sound = "ALERT3",
			...
			inducewarn = {
				varname = format(L.alerts["%s Cast"],SN[64059]),
				type = "dropdown",
				text = format(L.alerts["%s Cast"],SN[64059]),
				time = 60,
				flashtime = 5,
				color1 = "BROWN",
				color2 = "MIDGREY",
			...
			squeezewarn = {
				varname = format(L.alerts["%s on others"],SN[64126]),
				type = "simple",
			...
			maladywarn = {
				varname = format(L.alerts["%s Warning"],SN[63830]),
				type = "simple",
				text = format("%s: #5#! %s",L.alerts["Malady"],L.alerts["MOVE AWAY"]),
				time = 3,
				sound = "ALERT5",
				color1 = "GREEN",
				flashscreen = true,
			...
			crushertentaclewarn = {
				varname = format(L.alerts["%s Spawns"],L.npc_ulduar["Crusher Tentacle"]),
				type = "dropdown",
				text = format(L.alerts["%s Spawns"],L.npc_ulduar["Crusher Tentacle"]).."!",
				time = "<crushertime>",
				flashtime = 7,
				color1 = "DCYAN",
				color2 = "INDIGO",
			...
			-- Phase 3
			empoweringshadowscd = {
				varname = format(L.alerts["%s Timer"],SN[64486]),
				type = "centerpopup",
				text = format(L.alerts["Next %s"],SN[64486]),
				time = 10, 
				flashtime = 5,
				color1 = "INDIGO",
				color2 = "RED",
			...
			shadowbeaconcd = {
				varname = format(L.alerts["%s Cooldown"],SN[64465]),
				type = "dropdown",
				text = format(L.alerts["Next %s"],SN[64465]),
				time = 45, 
				flashtime = 5,
				color1 = "BLUE",
				icon = ST[64465],
			...
			deafeningcd = {
				varname = format(L.alerts["%s Cooldown"],SN[64189]),
				type = "dropdown",
				text = format(L.alerts["Next %s"],SN[64189]),
				time = 60,
				flashtime = 5,
				color1 = "BROWN",
				icon = ST[64189],
			...
			deafeningwarn = {
				varname = format(L.alerts["%s Cast"],SN[64189]),
				type = "centerpopup",
				text = format(L.alerts["%s Cast"],SN[64189]),
				time = 2.3,
				color1 = "ORANGE",
				sound = "ALERT5",
				icon = ST[64189],
			...
			lunaticgazewarn = {
				varname = format(L.alerts["%s Cast"],SN[64163]),
				type = "centerpopup",
				text = format("%s! %s!",SN[64163],L.alerts["LOOK AWAY"]),
				time = 4,
				color1 = "PURPLE",
				sound = "ALERT1",
				flashscreen = true,
			...
			lunaticgazecd = {
				varname = format(L.alerts["%s Cooldown"],SN[64163]),
				type = "dropdown",
				text = format(L.alerts["%s Cooldown"],SN[64163]),
				time = 11,
				flashtime = 5,
				color1 = "GREEN",
				color2 = "YELLOW",
