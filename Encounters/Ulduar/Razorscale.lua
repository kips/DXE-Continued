do
	...
	local data = {
		...
		alerts = {
			...
			devourwarnself = {
				varname = format(L.alerts["%s on self"],SN[63236]),
				type = "simple",
				text = format("%s: %s! %s!",SN[63236],L.alerts["YOU"],L.alerts["MOVE"]),
				time = 1.5,
				color1 = "RED",
				sound = "ALERT1",
				flashscreen = true,
			...
			breathwarn = {
				varname = format(L.alerts["%s Cast"],SN[63317]),
				type = "centerpopup",
				text = format(L.alerts["%s Cast"],SN[63317]),
				time = 2.5,
				flashtime = 2.5,
				color1 = "BLUE",
				color2 = "WHITE",
			...
			chaindur = {
				varname = format(L.alerts["%s Duration"],L.alerts["Chain"]),
				type = "centerpopup",
				text = format(L.alerts["%s Duration"],L.alerts["Chain"]),
				time = 38,
				color1 = "BROWN",
				sound = "ALERT3",
				icon = ST[60540],
			...
			permlandwarn = {
				varname = format(L.alerts["%s Warning"],L.alerts["Permanent Landing"]),
				type = "simple",
				text = format(L.alerts["%s Permanently Landed"],L.npc_ulduar["Razorscale"]).."!",
				time = 1.5,
				sound = "ALERT4",
				icon = ST[45753],
			},
			harpoonwarn = {
				varname = format(L.alerts["%s Warning"],SN[43993]),
				type = "simple",
				text = format(L.alerts["%s Ready"],SN[43993]).."!",
				time = 3,
				sound = "ALERT5",
				color1 = "ORANGE",
				icon = ST[56570],
